<script lang="ts">
  ...
  $: certifiedMachines = routeData
    .filter(
      (m) => (m.status === filter.status || filter.status === null) && (m.tier === filter.tier || filter.tier === null)
    )
    .sort((a, b) => {
  ...
  function displayStatus(machine: DashboardMachine): MachineStatus {
    ...
    return isWorking(machine) ? MachineStatus.WORKING : MachineStatus.IDLE;
  }
...
{#if routeData && permissions}
  <div class="page">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="join join-horizontal">
            <input
              ...
              aria-label="Tier 1"
              class="btn join-item"
              checked={filter.tier === 1}
              on:click={() => setFilter({ tier: 1 })}
            />
            <input
              data-sveltekit-reload
              type="radio"
              ...
              aria-label="Tier 2"
              class="btn join-item"
              checked={filter.tier === 2}
              on:click={() => setFilter({ tier: 2 })}
            />
            <input
              data-sveltekit-reload
              type="radio"
              ...
              aria-label="Tier 3"
              class="btn join-item"
              checked={filter.tier === 3}
              on:click={() => setFilter({ tier: 3 })}
            />
            <input
              data-sveltekit-reload
              type="radio"
              ...
              aria-label="Any"
              class="btn join-item"
              checked={filter.tier === null}
              on:click={() => setFilter({ tier: null })}
            />
          </div>
        </div>
        <div class="flex flex-col gap-2">
          ...
          <div class="join join-horizontal">
            <input
              ...
              aria-label="Ready"
              class="btn join-item"
              checked={filter.status === MachineStatus.IDLE}
              on:click={() => setFilter({ status: MachineStatus.IDLE })}
            />
            <input
              type="radio"
              name="options2"
              aria-label="Busy"
              class="btn join-item"
              checked={filter.status === MachineStatus.WORKING}
              on:click={() => setFilter({ status: MachineStatus.WORKING })}
            />
            <input
              type="radio"
              name="options2"
              aria-label="Down"
              class="btn join-item"
              checked={filter.status === MachineStatus.FAULT}
              on:click={() => setFilter({ status: MachineStatus.FAULT })}
            />
            <input
              type="radio"
              name="options2"
              ...
              on:click={() => setFilter({ status: null })}
            />
          </div>
        </div>
      </div>
    ...
    {#key filter}
      <div
        class="static grid grid-col-1 lg:grid-cols-2 xl:grid-cols-3 md:gap-12"
          ...
          <div class="col-span-1 window !p-0 {getBackgroundStyles({ ...machine, status: displayStatus(machine) })}">
            <div class="flex flex-col gap-8 h-full w-full">
              <div class="flex flex-col grow gap-8 justify-start items-center">
                <div class="flex justify-between w-full border-b border-base-content/25">
                  <div class="p-4 flex items-center">
                    <div class="flex flex-col justify-center">
                      <span
                        class="text-xl text-center font-semibold {getStatusStyles({
                          ...machine,
                          status: displayStatus(machine)
                        })}"
                      >
                  ...
                  <div
                    class="text-xl p-4 whitespace-nowrap overflow-hidden text-ellipsis max-w-full {getStatusStyles({
                      ...machine,
                      status: displayStatus(machine)
                    })}"
                  >
                ...
                <div
                  class="aspect-square flex flex-col justify-center {displayStatus(machine) === MachineStatus.FAULT
                    ? 'opacity-25'
                    : ''}"
                >
                  <img src="/{machine.model}.png" alt="{machine.nickname} printer" class="h-36 w-36 md:h-48 md:w-48" />
                </div>
              ...
              <!-- Buttons -->
              <div
                class="relative join join-horizontal rounded-t-none rounded-box w-full border-t border-base-content/25"
              >
                <button
                  class="basis-1/4 grow h-full btn px-2 join-item !ml-0 border-none btn-primary"
                  disabled={displayStatus(machine) !== MachineStatus.IDLE ||
                    !isTierCertified(permissions, machine.tier)}
                ...
                <button
                  class="basis-1/4 grow h-full btn px-2 join-item !ml-0 border-0 btn-secondary"
                  disabled={!isWorking(machine) || !isTierCertified(permissions, machine.tier)}
                  on:click={() => cancelPrintModal.launchModal(machine)}
                >
                  <div class="my-2 gap-2 items-center justify-center flex flex-col">
                    <Octagon />
                    <div>Stop</div>
                ...
                <button
                  class="basis-1/4 grow h-full btn px-2 join-item !ml-0 border-0 btn-accent"
                  disabled={displayStatus(machine) === MachineStatus.FAULT ||
                    !isTierCertified(permissions, machine.tier)}
                  on:click={() => newIssueModal.launchModal(machine)}
                >
                  <div class="my-2 gap-2 items-center justify-center flex flex-col">
                    <Tag />
                    <div>Issue</div>
                ...
                {#if !isTierCertified(permissions, machine.tier)}
                  <div
                    class="absolute inset-0 flex justify-center items-center left-[1px] bg-gradient-to-t from-warning/75 to-warning/50 rounded-b-box"
                  >
