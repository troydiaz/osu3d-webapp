...
export const load = (async ({ params, locals: { supabase, getSession, getPermissions } }) => {
  ...
  const { data: inventoryLocations } = await supabase.from('inv_locations').select(`*`).returns<InventoryLocation[]>();

  if (!inventoryLocations) throw error(404, `Resource not found`);
