...
export const actions = {
  changePermsMulti: async ({ request, locals: { supabase, getSession } }) => {
    ...
    let newPermsList: { id: string; newPermLevel: number; success: boolean }[] = JSON.parse(
      formData.get('permArray') as string
    ).map((data: { id: string; newPermLevel: number }) => {
      return { ...data, success: false };
    });

    for (let permChange of newPermsList) {
      let check = await supabase.from('user_levels').select('*').eq('user_id', permChange.id).single();
