...
<div class="flex flex-row items-center self-end">
  <button
    class="btn btn-sm btn-ghost disabled:bg-transparent"
    on:click={() => changePage(currentPage - 1)}
    disabled={lowerIndex === 0}
  >
    <ArrowLeft class="outline-none" />
  </button>
  <span class="w-20 md:w-36 text-center"
    ><span class="hidden md:inline">Entries</span> {upperIndex === 0 ? 0 : lowerIndex + 1} — {upperIndex}</span
  >
  <button
    class="btn btn-sm btn-ghost disabled:bg-transparent"
    on:click={() => changePage(currentPage + 1)}
    disabled={upperIndex === totalRows}
  >
