...
<div class="relative">
  <SidebarTooltip activeRoute={$page.url.pathname.startsWith(url)}>
    <li class="group h-14">
      ...
      <div class="absolute inset-0 bg-gradient-to-b rounded-md transition-none from-blue-300 to-blue-400 dark:from-blue-400 dark:to-blue-500 {$page.url.pathname.startsWith(url) ? 'opacity-100 group-hover:rounded-r-none' : 'group-hover:opacity-100 opacity-0 rounded-r-none'}"></div>
