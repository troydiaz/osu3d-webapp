<script lang="ts">
  ...
  import { detectOverflow, offset, shift } from '@floating-ui/dom';
  ...
  const [floatingRef, floatingContent] = createFloatingActions({
    ...
    middleware: [offset({ crossAxis: -15, mainAxis: 35 }), arrow({ element: arrowRef }), middleware],
...
{#if showTooltip}
  <div class="fixed z-20" use:floatingContent>
    <div
      class="lg:hidden flex w-80 rounded-2xl bg-warning text-error-content p-4 transition flex-col gap-2"
      transition:fly={{ x: -25, duration: 200 }}
    >
      <!-- Arrow -->
    ...
    <div class="hidden lg:flex w-80 rounded-2xl bg-warning text-error-content p-4 transition flex-col gap-2">hello</div>
  </div>
