<script lang="ts">
  ...
  const [floatingRef, floatingContent] = createFloatingActions({
    strategy: 'absolute',
    placement: 'bottom',
    middleware: [offset({ crossAxis: 25, mainAxis: 15 }), arrow({ element: arrowRef })],
...
<div
  ...
  on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && (showTooltip = !showTooltip)}
  class={showTooltip ? 'z-20' : ''}
>
  <slot />
</div>
...
{#if showTooltip}
  <div class="absolute w-80 z-20" use:floatingContent>
    ...
    >
      ...
      <form id="submit-new-item" method="post" action="?/submitNewItem" />
      ...
      <div class="form-control w-full max-w-xs">
        <label for="inv_category_id" class="label"
          ><span class="label-text text-primary-content font-light">Category</span></label
        >
      ...
      <div class="form-control w-full max-w-xs">
        <label for="minimum" class="label"
          ><span class="label-text text-primary-content font-light">Min. threshold (optional)</span></label
        >
        <input form="submit-new-item" type="number" name="minimum" class="input font-light" />
      </div>

      <div class="flex flex-row justify-end gap-4 mt-4">
        <button class="btn btn-sm btn-ghost text-primary-content font-light" form="submit-new-item" type="submit"
          >Submit</button
        >
  ...
  <button
    class="fixed w-screen h-screen top-0 left-0 bg-neutral-900/50 z-10"
