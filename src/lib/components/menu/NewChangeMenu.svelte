...
{#if showTooltip}
  <div class="absolute w-52 z-20" use:floatingContent>
    <div
      class="w-full rounded-2xl p-4 transition flex flex-col items-center justify-center gap-2 {mode === 'add'
        ? 'bg-success'
        : 'bg-error'}"
    ...
    >
      ...
      <form id="submit-new-change" method="post" action="?/submitNewChange" />

      <!-- New Item Dialog -->
      <div class="text-xl text-bold text-black text-center font-thin">
      ...
      <div class="form-control w-1/2">
        <input
          form="submit-new-change"
          type="number"
          name="amount"
          class="input text-3xl text-center font-light hide-input-arrows"
          bind:value={amount}
        />
      </div>
