...
<div class="flex flex-col space-y-4">
  <div class="flex flex-row justify-start items-center window-header">
    ...
    <button
      class="btn btn-sm btn-success ml-4 md:ml-0"
      form="fault-resolve-form"
      type="submit"
      disabled={faults.filter((f) => f.checked).length === 0}>Resolve</button
    >
    <div class="grow"></div>
      ...
      <table class="table">
        ...
        <tbody>
          {#each faults.slice(lowerIndex, upperIndex) as fault, i}
            <tr
              class="transition {fault.checked ? 'bg-success text-success-content' : ''}"
              class:cursor-pointer={!fault.resolved}
              on:click={() => (fault.resolved ? null : (fault.checked = !fault.checked))}
            >
              <td
                ><input
                  type="checkbox"
                  class="checkbox bg-base-100 disabled:bg-opacity-25 align-middle"
                  bind:checked={fault.checked}
                  disabled={fault.resolved}
                /></td
              >
              <td
                ><span class="hidden md:inline">{getDateAndTime(fault.created_at, true)}</span><span class="md:hidden"
                  >{getDateAndTime(fault.created_at, false)}</span
                ></td
              >
              <td
                ><code class="p-1 rounded-lg uppercase {getEventTypeColor(fault.event_type)}">{fault.event_type}</code
                ></td
              >
              <td class="hidden md:table-cell">{fault.created_by?.full_name ?? '—'}</td>
              <td>
                <div
                  class="tooltip before:whitespace-pre-wrap before:content-[attr(data-tip)]"
                  data-tip={fault.description ?? '—'}
                >
