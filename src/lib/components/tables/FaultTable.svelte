    ...
    <table class="table table-compact min-w-full shadow-lg bg-base-100 outline outline-1 outline-base-content/10 overflow-hidden">
        ...
        <tbody>
            {#each faults.slice(lowerIndex, upperIndex) as fault, i}
                <tr class="transition {fault.checked ? 'bg-success text-success-content' : 'bg-base-100 outline outline-1 outline-base-content/10 text-neutral-content'}" class:cursor-pointer={!fault.resolved} on:click={() => fault.resolved ? null : fault.checked = !fault.checked}>
