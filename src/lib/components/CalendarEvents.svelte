<script lang="ts">
  ...
  function getBorderStyle(date: Date) {
    ...
    if (isToday(date)) style += 'bg-gradient-to-t from-primary via-primary bg-primary';
    else if (!isSameMonth(selectedMonth, date)) style += 'bg-gradient-to-t from-neutral via-neutral bg-base-100';
    else style += 'bg-gradient-to-t from-gray-500 via-gray-500 bg-base-100';
  ...
  function getWeekendStyle(date: Date) {
    if (isWeekend(date))
      if (getDay(date) === 0) return ' mr-16';
      else return ' ml-16';
  }

  function getTextStyle(date: Date) {
    if (!isSameMonth(selectedMonth, date)) return 'text-base-content/25';
    else return 'text-base-content';
  }
  ...
  function getEventStyle(date: Date, type: 'weekly' | 'monthly') {
    if (type === 'weekly')
      if (isThisWeek(date)) return 'bg-warning text-warning-content';
      else if (isBefore(date, new Date())) return 'bg-error text-error-content';
      else return 'bg-success text-success-content';
    else if (isThisMonth(date)) return 'bg-warning text-warning-content';
    else if (isBefore(date, new Date())) return 'bg-error text-error-content';
    else return 'bg-success text-success-content';
  }
</script>
...
<div class="w-[95%] mx-auto grid grid-cols-7 justify-items-center text-xl font-thin select-none">
  <div class="mr-16">Sunday</div>
  <div>Monday</div>
  <div>Tuesday</div>
  <div>Wednesday</div>
  <div>Thursday</div>
  <div>Friday</div>
  <div class="ml-16">Saturday</div>

  {#each { length: totalCalendarDays + 1 } as _, i}
    <div class="w-full aspect-square p-1 {getWeekendStyle(addDays(firstCalendarDay, i))}">
      <div
        class="w-full aspect-square rounded-xl flex item-center justify-center p-0.5 shadow-lg shadow-base-300 {getBorderStyle(
          addDays(firstCalendarDay, i)
        )}"
      >
        <div class="relative aspect-square rounded-xl bg-base-100">
          <div class="text-end p-1 px-2 font-light {getTextStyle(addDays(firstCalendarDay, i))}">
            {getDate(addDays(firstCalendarDay, i))}
          </div>

          <!-- Events -->
          <div class="absolute top-9 left-0 w-full flex flex-col gap-1">
            {#each getEvents(addDays(firstCalendarDay, i)) as event}
              <NewMaintMenu {event} eventStyles={getEventStyle(addDays(firstCalendarDay, i), event.type)}>
                <div
                  class="cursor-pointer hover:opacity-100 opacity-75 transition text-xs text-center font-bold {getEventStyle(
                    addDays(firstCalendarDay, i),
                    event.type
                  )}"
                >
...
<div class="w-full flex flex-row justify-end items-center my-8">
  ...
  <div class="bg-success/80 text-success-content text-xs font-mono rounded rounded-l-none uppercase px-2 py-1">
    Upcoming
  </div>
</div>
